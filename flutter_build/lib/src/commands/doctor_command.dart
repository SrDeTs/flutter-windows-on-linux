    ...
    await log.group('Target project (cwd)', () async {
      try {
        ...
        log.kv({
          ...
          'windows/ scaffold':
              project.hasWindowsScaffold ? 'present' : 'MISSING',
          'Windows plugins': project.plugins.isEmpty
              ? '(none)'
              : project.plugins
                  .map((pl) =>
                      '${pl.name}${pl.hasNativeCode ? '' : '  (dart-only)'}')
                  .join(', '),
        });
  ...
  void _scanForRiskyPluginCode(
    ...
    for (final plugin in project.plugins) {
      ...
      for (final e in dir.listSync(recursive: true)) {
        ...
        if (!const {'.cpp', '.cc', '.cxx', '.h', '.hpp'}.contains(ext)) {
          continue;
        }
        ...
        for (final entry in patterns.entries) {
          if (txt.contains(entry.value)) {
            hits.add(
                '${entry.key} in ${p.relative(e.path, from: plugin.rootPath)}');
          }
        }
      }
